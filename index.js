		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD ⁩KIRITO BOT*',
			...
			const premium = ["557381044096@s.whatsapp.net","556296638900@s.whatsapp.net","557191467119@s.whatsapp.net","559885018147@s.whatsapp.net","5511974612292@s.whatsapp.net","554792091566@s.whatsapp.net"]
			...
			const mentions = (teks, memberr, id) => {
				(id == null || id == undefined || id == false) ? kirito.sendMess(from, teks.trim(), extendedText, {contextInfo: {"mentionedJid": memberr}}) : kirito.sendMess(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": memberr}})
			}
              //function leveling
            ...
            try {
                ...
                if (requiredXp <= getLevelingXp(sender)) {
                    ...
                    awat reply(`*「 TU SUBIU DE LEVEL MLKKK 」*\n\n➸ *Nome*: ${sender}\n➸ teu xpzin: ${getLevelingXp(sender)}\n➸ *Level*: ${getLevel} -> ${getLevelingLevel(sender)}\n\nBOAKKKK   `)
                }
            } catch (err) {
                console.error(err)
            }
...
const get = require('got')
	...
	if (messC.includes("fdp")){
		...
		if (messC.includes("kkkkkk")){
			...
			if (messC.includes("Ana")){
			dark.updatePresence(from, Presence.composing)
			reply("O Bruno te ama🙄👌")
	}
		...
		if (messC.includes("bot")){
			...
			dark.updatePresence(from, Presence.composing)
			axios.get = fs.readFileSync('./assets/CALA A BOCA,MACACO!.mp3');
            dark.sendMess(from, axios.get, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
		...
		case 'tahta':
				...
				case 'modoanime':
					...
					if ((args[0]) === 'on') {
						...
						anime.splice(from, 1)
						fs.writeFileSync('./database/json/anime.json', JSON.stringify(anime))
						reply(`✓Modo anime desativado com sucesso no grupo☜︎︎︎ *${groupMetadata.subject}*`)
					} else {
						reply('On para ligar, Off para desligar')
					}
					break
					case 'antiracismo':
					if (!isGroup) return reply(mess.only.group)
    ...
    break
		...
		case 'halloweentext':
         ...
         case 'moddroid':
			...
			hepi = data.result[0] 
			teks = `𝐍𝐎𝐌𝐄: ${data.result[0].title}\n𝐄𝐃𝐈𝐓𝐎𝐑: ${hepi.publisher}\n:𝐌𝐎𝐃 𝐈𝐍𝐅𝐎 ${hepi.mod_info}\n𝐓𝐀𝐌𝐀𝐍𝐇𝐎: ${hepi.size}\n𝐔𝐓𝐈𝐌𝐀 𝐕𝐄𝐑𝐒𝐀𝐎: ${hepi.latest_version}\n𝐆𝐄𝐍𝐄𝐑𝐎: ${hepi.genre}\n*link:* ${hepi.link}\n𝐃𝐎𝐖𝐍𝐋𝐎𝐀𝐃: ${hepi.download}`
...
// recursos anime, random @dark domina vida
          ...
          reply('Espere..')
				...
				if (!isUser) return reply(mess.only.userB)
					...
					dark.sendMess(from, sa, image, { quoted: mek, caption: '*╭────*「 *𝐔𝐒𝐔𝐀𝐑𝐈𝐎 𝐏𝐑𝐄𝐌𝐈𝐔𝐌👑* 」\n*│+ wa.me/5522996215481*╰──────*「 * 𝐊𝐈𝐑𝐈𝐓𝐎* 」*────*\n\n*_SE QUER SER UM USUÁRIO PREMIUM DO 𝐊𝐈𝐑𝐈𝐓𝐎 BOT_*\n*_Ketik #daftarvip*' })
					break
					case 'cekmod': 
				...
				case 'hentai':
					...
					dark.sendMess(from, bufferer, image, {quoted: mek, caption: 'Cara bate pra 2d kkkkkk,eu tbm bato rlxkkk'})
					break
				case 'bomdia':
					memeindodododododododododododododododo = awat kagApi.memeindododododododododododododododododo()
				...
				case 'dono':
					memeindodododododododododododododododo = awat kagApi.memeindododododododododododododododododo()
					bufferer = awat getbufferer(`https://WAbvtVry_400x400.jpg`)
					dark.sendMess(from, bufferer, image, {quoted: mek, caption: '*CRIADOR:* ꪶ͢𝐷𝛯び𝑆↯𝐾𝐼𝑅𝐼𝑇𝛩𖤐⁩᭄⃖ ☔\n*YOUTUBE:* off\n*WPP:* wa.me/+5573981044096\n*INSTA:* @sadzkkk\n\n\nEspero que tenham gostado do bot 🐊🚩️'})
					break
